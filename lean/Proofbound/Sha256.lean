...
A direct FIPS 180-4 implementation used when published bytes must be bound
inside the theorem boundary. `UInt32` arithmetic supplies modulo `2^32`.
-/
...
private def schedule (block : ByteArray) (offset : Nat) : Array UInt32 := Id.run do
  ...
  for i in [0:16] do w := w.push (beWord block (offset + i * 4))
  for i in [16:64] do
    w := w.push (w[i - 16]! + smallSigma0 (w[i - 15]!) + w[i - 7]! + smallSigma1 (w[i - 2]!))
  return w

private def compress (state : Array UInt32) (block : ByteArray) (offset : Nat) : Array UInt32 := Id.run do
...
def sha256Hex (bytes : ByteArray) : String := toHex (sha256 bytes)
