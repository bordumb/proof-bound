...
Canonical evidence version 4 also permits exact artifact observations without
changing the formal or linkage facets. `observation-inputs.schema.json` is the closed manifest used to
...
byte-recomputed publication. Evidence and release version 3 remain obsolete;
older schemas reject the contextual route.
