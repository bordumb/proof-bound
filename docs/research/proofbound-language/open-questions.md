...
EXP-LANG-019 preregisters exactly that repair. It retains the successful
Windows initialization boundary, requires a binary Python output path, and
proves each loopback endpoint reachable with the same staged subject before
the AppContainer attempt. Connection-refused, timeout, and unreachable remain
explicitly insufficient denial evidence.
