...
| EXP-0001-D03 | 0001 | Proofbound v0.5 has no manifest/adapter route that emits trusted-transcription evidence with its transcriber and re-encoder TCB nodes. | `spec-change (§7.1.1, §9.1.1, §11.2.1, v0.9.0); adr (#15); fixed` | [ADR 0015](../adr/0015-executable-trusted-transcription.md) |
...
| EXP-0002-D01 | 0002 | The Auths algebra translation manifest does not own its full invocation or output map; xtask constants remain authoritative. | `spec-change (§11.3, v0.8.0); adr (#14); fixed` | [ADR 0014](../adr/0014-authoritative-translation-manifests.md) |
| EXP-0002-D02 | 0002 | Template count checks cover declared files, but hardcoded inventory prevents fail-closed discovery of an undeclared template. | `spec-change (§11.3, v0.8.0); adr (#14); fixed` | [ADR 0014](../adr/0014-authoritative-translation-manifests.md) |
...
| EXP-0003-D02 | 0003 | Charon may exit zero while an inherent-method selector produces an empty translation inventory. | `spec-change (§10.2, §11.3, v0.10.0); adr (#16); fixed` | [ADR 0016](../adr/0016-exact-adapter-inventories.md) |
...
| EXP-0003-D05 | 0003 | Mutation receipts bind registered witnesses and input bytes but v0.5 does not automatically apply the registry's named mutant path. | `spec-change (§10.2, §11.2, §11.5, v0.11.0); adr (#17); fixed` | [ADR 0017](../adr/0017-sealed-singleton-mutation-replay.md) |
| EXP-0003-D06 | 0003 | A mutation-witness evidence unit spanning multiple mutations invalidates every attached claim when any one witness fails. | `spec-change (§10.2, §11.2, §11.5, v0.11.0); adr (#17); fixed` | [ADR 0017](../adr/0017-sealed-singleton-mutation-replay.md) |
...
| EXP-0004-D03 | 0004 | Registered model-check assumptions have no representation in a `/1` evidence receipt. | `spec-change (§9.7, §11.5, v0.7.0); adr (#13)` | [ADR 0013](../adr/0013-complete-version-2-receipt-fidelity.md) |
| EXP-0004-D04 | 0004 | Unknown peak memory is serialized as zero, conflating “not measured” with a measured zero-byte peak. | `spec-change (§11.5, §16.3, v0.7.0); adr (#13)` | [ADR 0013](../adr/0013-complete-version-2-receipt-fidelity.md) |
| EXP-0004-D05 | 0004 | Supplying reader-facing `public_language` replaces the receipt's internal statement, so the two statements cannot both be audited. | `spec-change (§6.3.2, §11.2, §11.5, v0.7.0); adr (#13)` | [ADR 0013](../adr/0013-complete-version-2-receipt-fidelity.md) |
| EXP-0004-D06 | 0004 | The adapter can observe several exact subprocess commands while core provenance retains only one representative command. | `spec-change (§10.2, §11.5, §16.1, v0.7.0); adr (#13)` | [ADR 0013](../adr/0013-complete-version-2-receipt-fidelity.md) |
| EXP-0004-D07 | 0004 | Canonical-artifact admission accepted checker-authored binding booleans even though the associated theorem did not mention the artifact digest. | `spec-change (§7.1, §9.4, v0.7.0); adr (#12)` | [ADR 0012](../adr/0012-derive-artifact-binding-from-theorem-content.md) |
