...
static void install_landlock(const char *runtime, const char *loader,
                             const char *source, const char *input,
                             const char *ephemeral) {
...
int main(int argc, char **argv) {
    ...
    if (argc < 8) {
        fprintf(stderr,
                "usage: linux-loader-enforcer RUNTIME LOADER SOURCE INPUT EPHEMERAL "
                "ARG...\n");
        return 125;
    }
    ...
    const char *ephemeral = argv[5];
    install_landlock(runtime, loader, source, input, ephemeral);
    install_seccomp();
    if (clearenv() != 0 || setenv("PB_REGISTERED_VALUE", "registered-env", 1) != 0)
        fail("environment");
    execv(runtime, &argv[6]);
    fail("runtime-exec");
    return 125;
}
