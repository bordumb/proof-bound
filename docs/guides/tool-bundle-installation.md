...
An exact response that reports mutable publication causes the workflow to
remove only its own release and tag. An uncertain publish response requires
operator inspection and never permits automatic deletion of a release that
GitHub might already have made immutable.
