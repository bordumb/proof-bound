"""Evaluate retained EXP-0020 Linux portability evidence."""

from __future__ import annotations

import json
from pathlib import Path
import sys
from typing import Any

from proofbound import linux_enforcement_execute as execution
from proofbound import linux_enforcement_research as research


EXECUTION_SCHEMA = "proofbound-research-linux-enforcement-execution/1"
EXPERIMENT = Path("docs/experiments/0020-linux-enforcement-portability")


def _nonblank_lines(path: Path) -> int:
    return sum(
        bool(line.strip()) for line in path.read_text(encoding="utf-8").splitlines()
    )


def evaluate_experiment(repository: Path) -> dict[str, Any]:
    """Derive the preregistered decision from retained artifacts.

    Args:
        repository: Proofbound repository root.

    Returns:
        Canonicalizable EXP-0020 decision record.

    Raises:
        ValueError: If retained evidence is inconsistent.
    """

    repository = repository.resolve()
    root = repository / EXPERIMENT
    results = root / "results"
    capture_bytes = (results / "capture.json").read_bytes()
    report = research.validate_capture_bytes(repository, capture_bytes)
    python_bytes = execution.canonical_json(report)
    rust_bytes = (results / "rust-report.json").read_bytes()
    retained_python = (results / "python-report.json").read_bytes()
    if python_bytes != rust_bytes or python_bytes != retained_python:
        raise ValueError("independent Linux reports differ")
    registration = json.loads((root / "preregistration.json").read_bytes())
    capture = json.loads(capture_bytes)
    ceilings = registration["ceilings"]
    rust_lines = _nonblank_lines(
        repository / "crates/proofbound-ir-prototype/src/linux_enforcement.rs"
    )
    python_lines = _nonblank_lines(
        repository / "python/proofbound/linux_enforcement_research.py"
    )
    disposition_names = [item[0] for item in report["effect_dispositions"]]
    q1 = len(disposition_names) == 9 and len(set(disposition_names)) == 9
    supported = report["availability"] == "supported"
    q2 = (
        supported
        and report["metrics"]["positive_executions"] == 30
        and report["metrics"]["authority_probe_executions"] == 21
        and report["metrics"]["denied_reusable"] == 0
        and not report["metrics"]["reviewed_tree_changed"]
    )
    q3 = (
        not supported
        and capture["slots"] == []
        and capture["platform"]["probe_exit_code"] != 0
        and capture["platform"]["landlock_abi"] is None
        and not capture["container_confinement_counted"]
    )
    q4 = (
        python_bytes == rust_bytes
        and len(report["policy_attacks"]) == registration["inventory"]["policy_attacks"]
        and all(item["exact"] for item in report["policy_attacks"])
    )
    q5 = (
        rust_lines <= ceilings["max_rust_lines"]
        and python_lines <= ceilings["max_python_lines"]
        and len(python_bytes) <= ceilings["max_report_bytes"]
    )
    if not supported and q1 and q3 and q4 and q5:
        decision = "unanswered"
    elif all((q1, q2, q3, q4, q5)):
        decision = "pass"
    else:
        decision = "revise"
    return {
        "schema": EXECUTION_SCHEMA,
        "experiment": "EXP-0020",
        "programme_experiment": "EXP-LANG-013",
        "decision": decision,
        "availability": report["availability"],
        "questions": {
            "Q1": {
                "passed": q1,
                "reason": "all nine backend-neutral authority classes have explicit Linux dispositions",
            },
            "Q2": {
                "passed": q2,
                "unavailable": not supported,
                "reason": "the Linux VM returned ENOSYS for the Landlock ABI probe; no workload ran",
            },
            "Q3": {
                "passed": q3,
                "reason": "unsupported availability produced zero receipts and did not count container confinement",
            },
            "Q4": {
                "passed": q4,
                "reason": "independent Rust and Python reports are byte-identical and all 16 attacks are exact",
            },
            "Q5": {
                "passed": q5,
                "reason": "implementation and report sizes remain below preregistered ceilings",
            },
        },
        "metrics": {
            **report["metrics"],
            "effect_dispositions": len(report["effect_dispositions"]),
            "policy_attack_rejections": len(report["policy_attacks"]),
            "rust_lines": rust_lines,
            "python_lines": python_lines,
            "canonical_report_bytes": len(python_bytes),
        },
        "identities": {
            "capture_sha256": execution.sha256_bytes(capture_bytes),
            "rust_report_sha256": execution.sha256_bytes(rust_bytes),
            "python_report_sha256": execution.sha256_bytes(python_bytes),
            "reports_equal": python_bytes == rust_bytes,
        },
    }


def main(argv: list[str] | None = None) -> int:
    """Write the canonical EXP-0020 evaluation."""

    arguments = sys.argv[1:] if argv is None else argv
    if len(arguments) != 2:
        print(
            "usage: linux_enforcement_experiment REPOSITORY EXECUTION", file=sys.stderr
        )
        return 2
    try:
        result = evaluate_experiment(Path(arguments[0]))
        Path(arguments[1]).write_bytes(execution.canonical_json(result))
    except (OSError, ValueError, research.LinuxEnforcementError) as issue:
        print(issue, file=sys.stderr)
        return 1
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
