"""Derive the preregistered EXP-0025 decision."""
...
EXECUTION_SCHEMA = "proofbound-research-windows-initialization-execution/1"
...
def evaluate(
    ...
    """Validate independent outputs and derive a fail-closed decision."""
    ...
    if expected_report != rust_report or expected_report != python_report:
        raise ValueError("independent EXP-0025 reports differ")
    if rust_attacks != python_attacks:
        raise ValueError("independent EXP-0025 attack reports differ")
...
def main(argv: list[str] | None = None) -> int:
    """Write one canonical EXP-0025 decision record."""

    arguments = sys.argv[1:] if argv is None else argv
    if len(arguments) != 7:
        print(
            "usage: windows_initialization_confirmation REPOSITORY CAPTURE "
            "RUST_REPORT PYTHON_REPORT RUST_ATTACKS PYTHON_ATTACKS EXECUTION",
            file=sys.stderr,
        )
        return 2
    try:
