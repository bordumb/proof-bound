"""Derive the preregistered EXP-0026 decision."""
...
EXECUTION_SCHEMA = "proofbound-research-windows-output-network-execution/1"
...
def evaluate(
    ...
    if expected_report != rust_report or expected_report != python_report:
        raise ValueError("independent EXP-0026 reports differ")
    if rust_attacks != python_attacks:
        raise ValueError("independent EXP-0026 attack reports differ")
...
def main(argv: list[str] | None = None) -> int:
    """Write one canonical EXP-0026 decision record."""

    arguments = sys.argv[1:] if argv is None else argv
    if len(arguments) != 7:
        print(
            "usage: windows_output_network_confirmation REPOSITORY CAPTURE "
