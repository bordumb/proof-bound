...
use proofbound_ir_prototype::{
    ...
    validate_windows_output_network_capture_bytes, validate_windows_wfp_attacks,
    validate_windows_wfp_capture_bytes,
};
...
fn main() -> ExitCode {
    ...
    if first.as_deref() == Some(std::ffi::OsStr::new("validate-windows-wfp")) {
        let Some(root) = args.next().map(PathBuf::from) else {
            eprintln!(
                "usage: proofbound-ir-prototype validate-windows-wfp <repository-root> <capture.json> <report.json>"
            );
            return ExitCode::from(2);
        };
        let Some(capture) = args.next().map(PathBuf::from) else {
        ...
        let result = std::fs::read(capture)
            .map_err(|error| error.to_string())
            .and_then(|bytes| {
                validate_windows_wfp_capture_bytes(&root, &bytes).map_err(|error| error.to_string())
            });
        return write_canonical_file_result(result, &output);
    }
    if first.as_deref() == Some(std::ffi::OsStr::new("validate-windows-wfp-attacks")) {
        let Some(root) = args.next().map(PathBuf::from) else {
            eprintln!(
                "usage: proofbound-ir-prototype validate-windows-wfp-attacks <repository-root> <attack-index.json> <report.json>"
            );
            return ExitCode::from(2);
        };
        let Some(index) = args.next().map(PathBuf::from) else {
        ...
        return write_canonical_file_result(
            validate_windows_wfp_attacks(&root, &index).map_err(|error| error.to_string()),
            &output,
        );
    }
