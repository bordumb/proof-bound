...
```json
{
  ...
  "execution": {
    "command": {
      ...
      "program": "/absolute/path/to/proofbound_lean_audit"
    },
    "completed_unix_ms": 2,
    "resource_usage": {
      "peak_disk_bytes": 0,
...
and hashes the compiled audit executable.
...
`Sha256Digest` members serialize as 64 lowercase hex because that is the core
type's current wire representation.
...
encoding and statement digest, attributed claim, environment, exact
foundational/project axioms, `contains_sorry_ax=false`, and the evaluation mode.
...
command/environment, timestamps, result/configuration identities, and declared
